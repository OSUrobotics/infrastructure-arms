...
class DoorArmController:
    def __init__(self):
        ...
        self.client_trajectory = actionlib.SimpleActionClient(action_address, kinova_msgs.msg.SetJointTrajectoryAction)
        ...
        self.send_joint_angle_target(
            [
                -0.780338353837345967,
                2.540778717977955,
                0.4260529439545537,
                5.5358591620936926,
                5.08,
                4.417817531159352,
                0.057345469084458735,
            ]
        )
        ...
        while True:
            ...
            pose_list = [self.generate_pose([0.46, -0.1, 0.32], [90, 0, 50])]
        ...
        while True:
            ...
            pose_list = [
                self.generate_pose([0.42, -0.07, 0.32], [90, 0, 50]),
                self.generate_pose([0.37, -0.03, 0.32], [90, 0, 47]),
                self.generate_pose([0.35, 0, 0.32], [90, 0, 43]),
                self.generate_pose([0.33, 0.03, 0.32], [90, 0, 40]),
            ]  # , self.generate_pose([.37,.1,.32], [90, 0, 50])]
        ...
        # print("Current pose: ", self.move_group.get_current_pose())
        pose_list = [
            self.generate_pose([0.35, 0.1, 0.32], [90, 0, 50])
        ]  # , self.generate_pose([.37,.1,.32], [90, 0, 50])]
    ...
    def validate_move(self, positions_tuple):
        ...
        current_joint_values = deepcopy(
            [
                math.radians(self.currentJointCommand[0]),
                math.radians(self.currentJointCommand[1]),
                math.radians(self.currentJointCommand[2]),
                math.radians(self.currentJointCommand[3]),
                math.radians(self.currentJointCommand[4]),
                math.radians(self.currentJointCommand[5]),
                math.radians(self.currentJointCommand[6]),
            ]
        )
