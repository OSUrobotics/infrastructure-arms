...
class GraspDatasetTrajectories(object):
    ...
    def __init__(self):
        ...
        try:
            ...
            # Create a publisher for the gripper position
            self.gripper_status_pub = rospy.Publisher("gripper_position", JointState, queue_size=10)
            # Create a callback timer to publish
            print("here")
            rospy.Timer(rospy.Duration(0.05), self.gripper_status_publish)
            print("here")
            # self.remove_box("base")
            self.add_box(pos=[0.75, 0, -0.05], dim=[2, 2, 0.1], name="based")
            rospy.sleep(1)
            # self.remove_box("cube")
            # self.add_box(pos=[.5,0,.05], dim=[.04,.04,.1], name="cube")
            # self.remove_box("cam_stand")
            self.add_box(pos=[0.62, 0, 0.9], dim=[0.1, 0.1, 0.5], name="cam_stand")
            # self.remove_box("backdrop")
            self.add_box(pos=[1.08, 0, 0.5], dim=[0.1, 2, 1], name="backdrop")
            # self.remove_box("top")
            self.add_box(pos=[0.08, 0, 1.1], dim=[1, 1, 0.1], name="top")
            self.add_box(pos=[0.75, 0, -0.05], dim=[2, 2, 0.1], name="based2")
            self.add_box(pos=[0, 0.41, 0], dim=[2, 0.1, 2], name="drape")

            self.generation_start_pose = self.get_cartesian_pose()
            self.generate_poses_top_XTRANS()  # 10
            # self.generate_poses_top_YTRANS() # 10
            # self.generate_poses_top_XROT() # 10
            # self.generate_poses_top_ZROT() # 10
            # self.generate_poses_top_YROT() # 10
            # self.generate_poses_side_XROT() # 20
            # self.generate_poses_side_ZROT() # 10
            # self.generate_poses_side_XTRANS() # 10
    ...
    def gripper_status_publish(self, blank):
        msg = JointState()
        msg.header.stamp = rospy.Time.now()
        msg.position.append(self.gripper_group.get_current_joint_values()[0])
        self.gripper_status_pub.publish(msg)

    def generate_start_poses(self):
        self.arm_group.set_end_effector_link("tool_frame")
        # Start Position
        start_point = [0.583, 0, 0.1]
    ...
    def start_arm_sequence_callback(self, goal):
        ...
        self.arm_group.set_end_effector_link("tool_frame")
        self.add_box(pos=[0.583, 0, 0.05], dim=[0.04, 0.04, 0.1], name="cube")

        if success:
            rospy.loginfo("Reaching Named Target Home...")
            success = self.reach_named_position("home")
        if success:  # and self.pose_num > 49:
        ...
        self.remove_box("cube")
        self.add_box(pos=[0.583, 0, 0.05], dim=[0.02, 0.02, 0.08], name="cube")
        if success:
    ...
    def test_pose_list(self):
        ...
        for i in range(5):
            ...
            self.add_box(pos=[0.575, 0, 0.05], dim=[0.025, 0.025, 0.105], name="cube")
            rospy.loginfo("Reaching Named Target Home...")
            success = self.reach_named_position("home")
            if success:
        ...
        for i in range(4):
            if i == 0:
                ...
                self.pose_list = []
                self.generate_poses_top_ZROT()
            self.add_box(pos=[0.575, 0, 0.05025], dim=[0.04, 0.04, 0.12], name="cube")
            self.add_box(pos=[0.58, 0, 0.05], dim=[0.025, 0.025, 0.105], name="cube")
            rospy.loginfo("Reaching Named Target Home...")
            success = self.reach_named_position("home")
            if success:
    ...
    def execute_poses(self):
        success = self.is_init_success
        self.pose_list = []
        self.generate_poses_top_XTRANS()  # 11
        print("made it 1")
        self.generate_poses_top_YTRANS()  # 11
        print("made it 2")
        self.generate_poses_top_XROT()  # 11
        print("made it 3")
        self.generate_poses_top_ZROT()  # 11
        print("made it 4")
        self.generate_poses_top_YROT()  # 11
        print("made it 5")
        self.generate_poses_side_XROT()  # 21
        print("made it 6")
        self.generate_poses_side_ZROT()  # 11
        print("made it 7")
        self.generate_poses_side_XTRANS()  # 11
        print("made it 8")
        self.generate_poses_side_YTRANS()  # 11
        print("made it 9")
        self.remove_box("cube")
        rospy.loginfo("Opening the gripper...")
        self.reach_gripper_position(0)
        ...
        for i in self.pose_list:
            self.add_box(pos=[0.585, 0, 0.05], dim=[0.03, 0.03, 0.08], name="cube")
            if success:
                rospy.loginfo("Reaching Named Target Home...")
                success = self.reach_named_position("home")
            if success:
    ...
    def set_cube_in_goal(self):
        actual_pose = self.get_cartesian_pose()
        actual_pose.position.x = 0.58
        actual_pose.position.y = 0.25
        actual_pose.position.z = 0.13
        waypoints = []
    ...
    def generate_poses_top_XTRANS(self, lower=-0.03, upper=0.03, start_point=[0.583, 0, 0.1], num=10):
    ...
    def generate_poses_top_YTRANS(self, lower=-0.03, upper=0.03, start_point=[0.583, 0, 0.1], num=10):
        step_size = (abs(upper) + abs(lower)) / num
        for i in range(num + 1):
            actual_pose = deepcopy(self.generation_start_pose)
            actual_pose.position.x = start_point[0]
    ...
    def generate_poses_top_YROT(self, lower=0, upper=90, start_point=[0.583, 0, 0.07], num=10):
        step_size = (abs(upper) + abs(lower)) / num
        for i in range(num + 1):
            actual_pose = deepcopy(self.generation_start_pose)
            actual_pose.position.x = start_point[0]
    ...
    def generate_poses_top_ZROT(self, lower=-50, upper=50, start_point=[0.583, 0, 0.1], num=10):
        step_size = (abs(upper) + abs(lower)) / num
        for i in range(num + 1):
            actual_pose = deepcopy(self.generation_start_pose)
            actual_pose.position.x = start_point[0]
    ...
    def generate_poses_top_XROT(self, lower=-40, upper=40, start_point=[0.583, 0, 0.1], num=20):
    ...
    def generate_poses_side_YTRANS(self, lower=-0.03, upper=0.03, start_point=[0.57, 0, 0.07], num=10):
        step_size = (abs(upper) + abs(lower)) / num
        for i in range(num + 1):
            actual_pose = deepcopy(self.generation_start_pose)
            actual_pose.position.x = start_point[0]
    ...
    def generate_poses_side_XTRANS(self, lower=-0.05, upper=0.03, start_point=[0.57, 0, 0.07], num=10):
        step_size = (abs(upper) + abs(lower)) / num
        for i in range(num + 1):
            actual_pose = deepcopy(self.generation_start_pose)
            actual_pose.position.x = start_point[0] + lower + (step_size * i)
    ...
    def generate_poses_side_XROT(self, lower=-30, upper=30, start_point=[0.57, 0, 0.07], num=10):
        step_size = (abs(upper) + abs(lower)) / num
        for i in range(num + 1):
            actual_pose = deepcopy(self.generation_start_pose)
            actual_pose.position.x = start_point[0]
    ...
    def generate_poses_side_ZROT(self, lower=-30, upper=30, start_point=[0.57, 0, 0.07], num=10):
        step_size = (abs(upper) + abs(lower)) / num
        for i in range(num + 1):
            actual_pose = deepcopy(self.generation_start_pose)
            actual_pose.position.x = start_point[0]
