...
class GraspDatasetTrajectories(object):
  ...
  def start_arm_sequence_callback(self, goal):
    ...
    self.remove_box("cube")
    self.add_box(pos=[.583,0,.05], dim=[.02,.02,.08], name="cube")
    if success:
  ...
  def generate_poses_top_XTRANS(self, lower=-.03, upper=.03, start_point=[.583,0,.1], num=10):
  ...
  def generate_poses_top_YTRANS(self, lower=-.03, upper=.03, start_point=[.583,0,.1], num=10):
    step_size = (abs(upper) + abs(lower)) / num
    for i in range(num+1):
      actual_pose = deepcopy(self.generation_start_pose)
      actual_pose.position.x = start_point[0]
  ...
  def generate_poses_top_YROT(self, lower=0, upper=90, start_point=[.583,0,.07], num=10):
    step_size = (abs(upper) + abs(lower)) / num
    for i in range(num+1):
      actual_pose = deepcopy(self.generation_start_pose)
      actual_pose.position.x = start_point[0]
  ...
  def generate_poses_top_ZROT(self, lower=-50, upper=50, start_point=[.583,0,.1], num=10):
    step_size = (abs(upper) + abs(lower)) / num
    for i in range(num+1):
      actual_pose = deepcopy(self.generation_start_pose)
      actual_pose.position.x = start_point[0]
  ...
  def generate_poses_top_XROT(self, lower=-40, upper=40, start_point=[.583,0,.1], num=20):
