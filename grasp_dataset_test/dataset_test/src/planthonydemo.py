...
class GraspDatasetTrajectories(object):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.add_box(pos=[0.5, 0, 0.9], dim=[0.1, 0.1, 0.5], name="cam_stand")
            # self.remove_box("backdrop")
            self.add_box(pos=[1, 0, 0.5], dim=[0.1, 2, 1], name="backdrop")
            # self.remove_box("top")
            self.add_box(pos=[0, 0, 1.1], dim=[1, 1, 0.1], name="top")
            self.add_box(pos=[0.75, 0, -0.05], dim=[2, 2, 0.1], name="base")
    ...
    def planthony_callback(self, request):
        ...
        self.reach_cartesian_pose(actual_pose, tolerance=0.01, constraints=None)
    ...
    def execute_poses(self):
        ...
        if success:
            for i in self.pose_list:
                self.add_box(pos=[0.55, 0, 0.05], dim=[0.04, 0.04, 0.1], name="cube")
                if success:
    ...
    def generate_poses_top_YROT(self, lower=0, upper=90, start_point=[0.55, 0, 0.09], num=5):
    ...
    def generate_poses_top_ZROT(self, lower=-20, upper=20, start_point=[0.55, 0, 0.1], num=5):
        step_size = (abs(upper) + abs(lower)) / num
        for i in range(num + 1):
            actual_pose = deepcopy(self.get_cartesian_pose())
            actual_pose.position.x = start_point[0]
    ...
    def generate_poses_top_XROT(self, lower=-20, upper=20, start_point=[0.55, 0, 0.09], num=5):
