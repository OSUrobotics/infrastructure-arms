...
def generate_code(request, response):
    ...
    for package in packages_dict.values():
        for s in ["srv", "msg"]:
            shutil.rmtree(
                "../{}/generated/{}".format(s, package.short_name_lowercase_with_underscores), ignore_errors=True
            )
        ...
        for item, package in traverse(proto_file):
            ...
            if isinstance(item, DescriptorProto):
                ...
                duplicated_fields = filter(
                    lambda x: x.type_name.split(".")[-1].casefold() in duplicated_messages_unordered_set, item.field
                )
                ...
                if message_contains_one_of:
                    ...
                    for member in item.field:
                        ...
                        if member.HasField("oneof_index"):
                            is_field_duplicated = (
                                member.type_name.split(".")[-1].casefold() in duplicated_messages_unordered_set
                            )
            ...
            if isinstance(item, ServiceDescriptorProto):
                ...
                for rpc in item.method:
                    ...
                    is_input_type_duplicated = (
                        rpc.input_type.split(".")[-1].casefold() in duplicated_messages_unordered_set
                    )
                    is_output_type_duplicated = (
                        rpc.output_type.split(".")[-1].casefold() in duplicated_messages_unordered_set
                    )
    ...
    for package in packages_dict.values():
        ...
        for enum in package.enums:
            ...
            ros_enum_path = os.path.join(
                "..", "msg/generated/{}{}{}.msg".format(enum.containing_folder, enum.prepend_message_name, enum.name)
            )
        ...
        for message in package.messages:
            ...
            for detailed_one_of in message.one_of_list:  # not empty
                ...
                ros_oneofPath = os.path.join(
                    ...
                    "msg/generated/{}{}{}_{}.msg".format(
                        message.containing_folder, message.prepend_message_name, message.name, detailed_one_of.name
                    ),
                )
            ...
            ros_messagePath = os.path.join(
                ...
                "msg/generated/{}{}{}.msg".format(
                    message.containing_folder, message.prepend_message_name, message.name
                ),
            )
        ...
        if package.messages:  # package contains at least one message
            ...
            current_header_filename = "kortex_driver/generated/robot/{}_proto_converter.h".format(
                package.short_name.lower()
            )
            this_package_context.include_file_names = filter(
                lambda x: "proto_converter" in x and x != current_header_filename, include_file_names
            )
            ...
            with open(
                os.path.join("..", "src/generated/robot/{}_proto_converter.cpp".format(package.short_name.lower())),
                "wt",
            ) as converterFile:
                converterFile.write(render("../templates/proto_converter.cpp.jinja2", this_package_context.__dict__))
            # ROS converter header file
            current_header_filename = "kortex_driver/generated/robot/{}_ros_converter.h".format(
                package.short_name.lower()
            )
            this_package_context.include_file_names = filter(
                lambda x: "ros_converter" in x and x != current_header_filename, include_file_names
            )
            with open(os.path.join("..", "include/" + current_header_filename), "wt") as converterFile:
            ...
            this_package_context.current_header_filename = current_header_filename
            with open(
                os.path.join("..", "src/generated/robot/{}_ros_converter.cpp".format(package.short_name.lower())), "wt"
            ) as converterFile:
        ...
        if package.methods:  # package contains at least one RPC
            ...
            current_header_filename = "kortex_driver/generated/interfaces/{}_services_interface.h".format(
                package.short_name.lower()
            )
            ...
            with open(
                os.path.join("..", "src/generated/robot/{}_services.cpp".format(package.short_name.lower())), "wt"
            ) as services_file:
                services_file.write(render("../templates/services_robot.cpp.jinja2", this_package_context.__dict__))

            # Generate simulation files
            current_header_filename = "kortex_driver/generated/simulation/{}_services.h".format(
                package.short_name.lower()
            )
            this_package_context.current_header_filename = current_header_filename
            this_package_context.include_file_names = include_file_names
            ...
            with open(
                os.path.join("..", "src/generated/simulation/{}_services.cpp".format(package.short_name.lower())), "wt"
            ) as services_file:
                services_file.write(
                    render("../templates/services_simulation.cpp.jinja2", this_package_context.__dict__)
                )
