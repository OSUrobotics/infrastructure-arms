...
for robot_name in robots:
    ...
    for joint in robot_joints:
        joints.append(joint)
        gains.update({joint: {"p": joint_p[i], "i": joint_i[i], "d": joint_d[i], "i_clamp": 10}})
        constraints.update({joint: {"trajectory": 0.05, "goal": 0.02}})
        i = i + 1
    joints = {"joints": joints}
    gains = {"gains": gains}
    constraints_dic = {"goal_time": 1.0, "stopped_velocity_tolerance": 0.02}
    ...
    robot_trajectory_position_controller_dic = {
        "effort_joint_trajectory_controller": robot_trajectory_position_controller
    }
    ...
    for joint in finger_joints:
        joints.append(joint)
        gains.update({joint: {"p": finger_p[i], "i": finger_i[i], "d": finger_d[i], "i_clamp": 1}})
        constraints.update({joint: {"trajectory": 0.05, "goal": 0.02}})
        i = i + 1
    joints = {"joints": joints}
    gains = {"gains": gains}
    ...
    robot_trajectory_position_controller.update(constraints_dic)
    robot_trajectory_position_controller_dic = {
        "effort_finger_trajectory_controller": robot_trajectory_position_controller
    }
  ...
  """
    ...
    i = 0
    for joint in robot_joints:
        robot_joint_position_controller = {
            "joint_"
            + str(i + 1)
